...
def apply_decoherence_to_state(alpha, beta, qubit_type, last_change_time, strength=0.05, time_decay_factor=0.1):
    ...
    if prob0 > prob1: # Closer to |0>
        ...
        if abs(alpha) > 1e-9:
           ...
           new_target_alpha = cmath.sqrt(1 - abs(new_target_beta)**2) + 0j # Ensure complex
        ...
        if abs(beta) > 1e-9:
            ...
            new_target_beta = cmath.sqrt(1 - abs(new_target_alpha)**2) + 0j # Ensure complex

        if not cmath.isclose(alpha, new_target_alpha): changed = True

    if changed:
        # print(f"Decoherence applied (decay ~{decay_factor:.3f})") # Debugging
        norm_alpha, norm_beta = normalize_state(new_target_alpha, new_target_beta)
        return norm_alpha, norm_beta, True
    else:
...
# Decoherence Toggle
deco_toggle = st.sidebar.checkbox(
...
# Update state only if changed to potentially trigger a rerun if needed
if deco_toggle != st.session_state.decoherence_enabled:
    st.session_state.decoherence_enabled = deco_toggle
    add_log(f"Decoherence {'enabled' if deco_toggle else 'disabled'}.")
    # Optional: Rerun if you want immediate feedback, though it might not be necessary
    # st.rerun()


# --- Decoherence Application Point ---
# Apply decoherence based on time elapsed *before* processing button clicks
# This happens implicitly on each script run if enabled
deco_applied_msg = ""
if st.session_state.decoherence_enabled:
    ...
    if changed:
        ...
        # No rerun needed here, the change will reflect in the current draw
...
if st.sidebar.button("Measure Qubit"):
    # Ensure amplitudes are complex before calculating probabilities
    current_alpha = complex(st.session_state.alpha)
    current_beta = complex(st.session_state.beta)
    prob0, prob1 = get_probabilities(current_alpha, current_beta)
...
# Ensure alpha/beta are complex for display/calculations
display_alpha = complex(st.session_state.alpha)
display_beta = complex(st.session_state.beta)
prob0, prob1 = get_probabilities(display_alpha, display_beta)
...
with col_status:
    ...
    st.code(f"α = {format_complex(display_alpha)}\nβ = {format_complex(display_beta)}", language=None)
...
with col_viz:
    ...
    fig = go.Figure(
        data=[
            go.Bar(name='P(0)', x=['|0⟩'], y=[prob0], marker_color='lightblue', text=f"{prob0:.3f}", textposition='auto'),
            go.Bar(name='P(1)', x=['|1⟩'], y=[prob1], marker_color='lightcoral', text=f"{prob1:.3f}", textposition='auto')
        ]
    )
    fig.update_layout(
        yaxis_range=[0, 1.05], # Ensure y-axis goes slightly above 1 for text display
        yaxis_title="Probability",
        xaxis_title="Basis State",
        title_text=f"Probability Distribution (Type: {st.session_state.qubit_type})",
        title_x=0.5,
        bargap=0.3, # Gap between bars of different categories
        # bargroupgap=0.1 # Not needed for single category per x-value
        uniformtext_minsize=8, # Ensure text is readable
        uniformtext_mode='hide', # Hide text if it doesn't fit
    )
    fig.update_traces(textangle=0) # Keep text horizontal

    st.plotly_chart(fig, use_container_width=True)

    # --- Log Display ---
    st.subheader("Action Log")
    st.text_area("Log:", value="\n".join(st.session_state.log), height=150, disabled=True, key="log_display")
...
# Force state_change_time update even if no button was pressed, for decoherence timing
# This ensures decoherence calculations use the time of the *last* interaction
# Put this at the very end to capture the time after all potential state changes in the script run
# st.session_state.state_change_time = time.time() # Re-evaluating if this is the best place. It might reset too often. Let's keep it tied to explicit actions for now.